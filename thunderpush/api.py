...
class ChannelHandler(ThunderApiHandler):
    @is_authenticated
    @is_json
    def post(self, messenger, channel):
    ...
    @is_authenticated
    def get(self, messenger, channel):
...
class UserHandler(ThunderApiHandler):
    ...
    @is_authenticated
    def delete(self, messenger, user):
...
urls = [
    (r'/api/1\.0\.0/(?P<apikey>.+)/users/', UserCountHandler),
    (r'/api/1\.0\.0/(?P<apikey>.+)/users/(?P<user>.+)/', UserHandler),
    (r'/api/1\.0\.0/(?P<apikey>.+)/channels/(?P<channel>.+)/', ChannelHandler),
]
